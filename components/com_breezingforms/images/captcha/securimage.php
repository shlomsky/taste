...
class Securimage {
	...
	*/
	...
	function Securimage()
	{
		...
		$this->ttf_file      = JPATH_SITE . '/components/com_breezingforms/images/captcha/elephant.ttf';

		$this->perturbation       = 0.9;
		$this->iscale             = 5;
		...
		$this->signature_font  = JPATH_SITE . '/components/com_breezingforms/images/captcha/elephant.ttf';
	...
	function validate()
	{
		if ( isset($_SESSION['securimage_code_value']) && !empty($_SESSION['securimage_code_value']) ) {
			if ( strtolower($_SESSION['securimage_code_value']) == strtolower(trim($this->code_entered)) ) {
				...
				if($_SESSION['secureimage_tries'] > 1){
					$_SESSION['securimage_code_value'] = '';  // clear code to prevent session re-use
					$_SESSION['secureimage_tries'] = 0;
				}
				$_SESSION['secureimage_tries']++;
			} else {
