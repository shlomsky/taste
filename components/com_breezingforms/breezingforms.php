 ...
 * or
 *     http://siteurl/index2.php?option=com_breezingforms......
 *
...
if(
	...
	if ($ok) {
		...
		// set by plugin
		if(isset($_SESSION['ff_editablePlg'.$form->name]) && $_SESSION['ff_editablePlg'.JRequest::getVar('ff_contentid','') . $form->name] != 0 && ( JRequest::getVar('ff_applic')=='plg_facileforms' || ( isset($ff_applic) && $ff_applic == 'plg_facileforms' )) ){
			$editable = $_SESSION['ff_editablePlg'.JRequest::getVar('ff_contentid','') . $form->name];
		}
		
		// set by plugin
		if(isset($_SESSION['ff_editable_overridePlg'.$form->name]) && $_SESSION['ff_editable_overridePlg'.JRequest::getVar('ff_contentid','') . $form->name] != 0 && ( JRequest::getVar('ff_applic')=='plg_facileforms' || ( isset($ff_applic) && $ff_applic == 'plg_facileforms' )) ){
			$editable_override = $_SESSION['ff_editable_overridePlg'.JRequest::getVar('ff_contentid','') . $form->name];
		}
		...
		if(( JRequest::getVar('ff_applic')=='mod_facileforms' || ( isset($ff_applic) && $ff_applic == 'mod_facileforms' )) ){
			...
			} else if(JFactory::getSession()->get('ff_editableMod'. JRequest::getVar('ff_module_id','') . $form->name, 0) != 0){
				$editable = JFactory::getSession()->get('ff_editableMod'.JRequest::getVar('ff_module_id','') . $form->name, 0);
			}
		}
		...
		if(( JRequest::getVar('ff_applic')=='mod_facileforms' || ( isset($ff_applic) && $ff_applic == 'mod_facileforms' )) ){
			...
			} else if(JFactory::getSession()->get('ff_editable_overrideMod'. JRequest::getVar('ff_module_id','') . $form->name, 0) != 0){
				$editable_override = JFactory::getSession()->get('ff_editable_overrideMod'.JRequest::getVar('ff_module_id','') . $form->name, 0);
			}
		}
		...
		if ($inframe && !$plainform) {
			...
			.'&amp;Itemid='.((JRequest::getInt( 'Itemid', 0) > 0 && JRequest::getInt( 'Itemid', 0) < 99999999) ? JRequest::getInt( 'Itemid', 0) : 1)
			.'&amp;ff_form='.$form->id
			.'&amp;ff_applic='.$ff_applic
					...
					if($file!="." && $file!=".."){
						...
						if(count($parts)>=5){
							if($parts[count($parts)-1] == 'flashtmp'){
								if (@file_exists($sourcePath.$file) && @is_file($sourcePath.$file) && @is_readable($sourcePath.$file) && @is_writeable($sourcePath.$file)){
...
} else if(JRequest::getBool('cfpassthru')){

	JRequest::setVar('format', 'raw');
	/**
	 * We need to get everything through curl cause dojo/js cannot retrieve the data from a different domain.
	 * So this is a transparent passthru
	 */
	$url = "http://www.crosstec.de/shareform.php";
	$ch = @curl_init();
	// set the target url
	...
	foreach($_REQUEST As $key => $value){
		if($key != 'option' && $key != 'task' && is_string($value)){
			@$postparams .= '&' . $key . '=' . urlencode($value);
		}
	}
	@curl_setopt($ch, CURLOPT_POSTFIELDS, "option=com_shareform&task=". JRequest::getVar('do') . $postparams);
	$result = @curl_exec ($ch);
	@curl_close ($ch);
	echo $result;
	...
	$list = $db->loadObjectList();
	if(count($list) == 0){
		BFRedirect(JURI::root(), BFText::_('Form does not exist!'));
		exit;
	}
	...
	$areas = Zend_Json::decode($form->template_areas);
	if(!is_array($areas)){
		BFRedirect(JURI::root(), BFText::_('Could not find PayPal data!'));
	}
	...
	foreach($areas As $area){
		...
		foreach($area['elements'] As $element){
			if($element['name'] == 'PayPalSelect' || $element['name'] == 'BfPaymentSelect'){
				if(isset($element['options']) && isset($element['options']['options']) && $element['options']['options'] != ''){
					$checkPP = false;
				}
				break;
			}
			...
			if($element['internalType'] == 'bfPayPal'){
				...
				if (strcmp ($lines[0], "SUCCESS") == 0) {
					...
					if ($checkPP && ( $keyarray['mc_gross'] != (doubleval($options['amount'])+doubleval($options['tax'])) || $keyarray['mc_currency'] != strtoupper($options['currencyCode']) ) ) {
						...
						$query = "SELECT * FROM #__facileforms_records WHERE id = '".JRequest::getInt('record_id', -1)."' And paypal_tx_id = '' LIMIT 1";
						$db->setQuery($query);
						...
						if (count($txid) != 0) {
							...
							if($txid[0]->paypal_tx_id == ''){
								...
								if( isset( $options['sendNotificationAfterPayment'] ) && $options['sendNotificationAfterPayment'] ){
									bf_sendNotificationBySession('bfPayedEmailNotification');
									bf_sendNotificationBySession('bfPayedMailbackEmailNotification');
								}
								...
								if($options['downloadableFile']){
									...
									if($options['thankYouPage'] != ''){
										...
										BFRedirect(JURI::root(), BFText::_('Thanks for paying with PayPal!'));
									}
								}
	...
	$areas = Zend_Json::decode($form->template_areas);
	if(!is_array($areas)){
		BFRedirect(JURI::root(), BFText::_('Could not find Payment data!'));
	}
				...
				if($options['downloadableFile']){
					...
					if(count($downloads) == 1){
						...
						if($downloads[0]->paypal_download_tries < $options['downloadTries']){
							...
							if(!file_exists($file)) {
								BFRedirect(JURI::root(), BFText::_('Could not find download fle!'));
							}
							...
							BFRedirect(JURI::root(), BFText::_('You reached the maximum amount of download tries!'));
						}
						
					} else {
						
						BFRedirect(JURI::root(), BFText::_('Download not possible, did you reload the page? Please contact the site administrator or the site support!'));
					}
					
				} else {

					BFRedirect(JURI::root(), BFText::_('This is no downloadable product!'));
				}
...
'.BFText::_('Please wait while we process your request...').'
</div>
</div>
</body>';
	...
	if($tx_token == ''){
		...
		if($formId != '' && $recordId != ''){
			...
			$areas = Zend_Json::decode($form->template_areas);
			if(!is_array($areas)){
				BFRedirect(JURI::root(), BFText::_('Could not find Sofortuebweisung data!'));
			}
					...
					if($element['internalType'] == 'bfSofortueberweisung'){
						...
						if($options['downloadableFile']){
							...
							if($options['thankYouPage'] != ''){
								...
								BFRedirect(JURI::root(), BFText::_('Thanks for paying with sofortüberweisung.de!'));
							}
						}
			...
			$msg = JText::_("Missing payment information. Not enough form information given.");
			$tx_token = JText::_("Not available");
				...
				if($hash == JRequest::getVar('hash','')){
					...
					if (count($txid) != 0) {
						...
						if($txid[0]->paypal_tx_id == ''){
							...
							$mailer->Subject = BFText::_('Your payment at sofortueberweisung.de');
							$mailer->Body 	 = BFText::_('Hallo,')."\n\n";
							$mailer->Body 	.= BFText::_('your payment succeeded, please see your payment details below.')."\n\n";
							$mailer->Body 	.= '--------------------------------------'."\n\n";
							$mailer->Body   .= BFText::_('Reason 1:').' '.JRequest::getVar('reason_1','')."\n";
							$mailer->Body   .= BFText::_('Reason 2:').' '.JRequest::getVar('reason_2','')."\n";
							$mailer->Body   .= BFText::_('Amount:').' '.str_replace('.',',',JRequest::getVar('amount','')).' '. JRequest::getVar('currency_id','') ."\n";
							$mailer->Body   .= BFText::_('Transaction:').' '.JRequest::getVar('transaction','')."\n";
							$mailer->Body   .= BFText::_('Account holder:').' '.JRequest::getVar('sender_holder','')."\n";
							$mailer->Body   .= BFText::_('Account number:').' '.JRequest::getVar('sender_account_number','')."\n";
							$mailer->Body   .= BFText::_('Bank code:').' '.JRequest::getVar('recipient_bank_code','')."\n";
							$mailer->Body   .= BFText::_('Bank name:').' '.JRequest::getVar('sender_bank_name','')."\n";
							$mailer->Body   .= BFText::_('BIC:').' '.JRequest::getVar('sender_bank_bic','')."\n";
							$mailer->Body   .= BFText::_('IBAN:').' '.JRequest::getVar('sender_iban','')."\n";
							$mailer->Body   .= BFText::_('Payment date:').' '.JRequest::getVar('created','')."\n\n";
							
							$mailer->Body 	.= '--------------------------------------'."\n\n";
							$mailer->Body 	.= BFText::_('Recipient of your payment.')."\n\n";
							$mailer->Body 	.= '--------------------------------------'."\n\n";
							
							$mailer->Body   .= BFText::_('Account holder:').' '.JRequest::getVar('recipient_holder','')."\n";
							$mailer->Body   .= BFText::_('Account number:').' '.JRequest::getVar('recipient_account_number','')."\n";
							$mailer->Body   .= BFText::_('Bank code:').' '.JRequest::getVar('recipient_bank_code','')."\n";
							$mailer->Body   .= BFText::_('Bank name:').' '.JRequest::getVar('recipient_bank_name','')."\n";
							$mailer->Body   .= BFText::_('BIC:').' '.JRequest::getVar('recipient_bank_bic','')."\n";
							$mailer->Body   .= BFText::_('IBAN:').' '.JRequest::getVar('recipient_iban','')."\n\n";
							
							$mailer->Body 	.= '--------------------------------------'."\n\n";
							
							$mailer->Body   .= BFText::_('Payment gateway: sofortueberweisung.de');
	...
	if (is_numeric(JRequest::getVar('form','')) && !empty($_FILES) && JRequest::getVar('bfFlashUploadTicket','') != '') {
		...
		if($formIdCount > 0){
			...
			if( @file_exists( $targetPath ) && @is_dir( $targetPath ) && @is_writeable( $targetPath ) ){
				...
				$targetFile = str_replace('//','/',$targetPath) . $_FILES['Filedata']['name'] . '_' . JRequest::getVar('itemName','') . '_' . JRequest::getVar('bfFlashUploadTicket') . '_' . $secureTicket . '_flashtmp';
