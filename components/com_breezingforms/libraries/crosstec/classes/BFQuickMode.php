...
class BFQuickMode{
	...
	function __construct( HTML_facileFormsProcessor $p ){
		...
		JFactory::getDocument()->addScript(JURI::root() . 'components/com_breezingforms/libraries/jquery/jquery.min.js');
		JFactory::getDocument()->addScript(JURI::root() . 'components/com_breezingforms/libraries/jquery/tooltip.js');

                if($this->useBalloonErrors){
                    JFactory::getDocument()->addStyleSheet( JURI::root() . 'components/com_breezingforms/libraries/jquery/validationEngine.jquery.css' );
                    JFactory::getDocument()->addScript(JURI::root() . 'components/com_breezingforms/libraries/jquery/jquery.validationEngine-en.js');
                    JFactory::getDocument()->addScript(JURI::root() . 'components/com_breezingforms/libraries/jquery/jquery.validationEngine.js');
                }
		...
		if($this->toggleFields != '[]'){
			...
			function bfDeactivateSectionFields(){
				for( var i = 0; i < bfDeactivateSection.length; i++ ){
					JQuery("#"+bfDeactivateSection[i]).find(".ff_elem").each(function(i){
			...
			function bfRegisterToggleFields(){
				for( var i = 0; i < toggleFieldsArray.length; i++ ){
					if( toggleFieldsArray[i].tCat == "element" || toggleFieldsArray[i].tCat == "section" ){
									...
									JQuery("[name=ff_nm_"+toggleField.sName+"[]]").blur(
										function(){
											for( var k = 0; k < thisToggleFieldsArray.length; k++ ){
												...
												if( 
													( ( regExp != "" && JQuery(this).val().match(regExp) > 0 ) || JQuery(this).val() == thisToggleFieldsArray[k].value ) && JQuery(this).get(0).name == "ff_nm_"+thisToggleFieldsArray[k].sName+"[]"														
												){
													thisBfToggleFields(thisToggleFieldsArray[k].state, thisToggleFieldsArray[k].tCat, thisToggleFieldsArray[k].tName, thisBfDeactivateField);
												}
											}
										}
									);
										 ...
										 JQuery("#" + JQuery("[name=ff_nm_"+toggleField.sName+"[]]").get(j).id ).click(										 	
										 	function(){
										 		...
										 		for( var k = 0; k < thisToggleFieldsArray.length; k++ ){
										 			...
										 			var chkGrpValues = new Array();
										 			if(JQuery(this).get(0).type == "checkbox" && JQuery(this).get(0).checked && thisToggleFieldsArray[k].value.beginsWith("#", true) && JQuery(this).get(0).name == "ff_nm_"+thisToggleFieldsArray[k].sName+"[]"){
										 				chkGrpValues = thisToggleFieldsArray[k].value.substring(1, thisToggleFieldsArray[k].value.length).split("|");
										 			}
										 			
											 		if( 
											 			// simple radio case for selected value
											 			( JQuery(this).get(0).type == "radio" && JQuery(this).val() == thisToggleFieldsArray[k].value && JQuery(this).get(0).name == "ff_nm_"+thisToggleFieldsArray[k].sName+"[]" )
											 			||
											 			...
											 			||
											 			// complex checkbox group case by multiple values
											 			(
											 				JQuery(this).get(0).type == "checkbox" && JQuery(this).get(0).checked && JQuery.inArray(JQuery(this).val(), chkGrpValues) != -1 && JQuery(this).get(0).name == "ff_nm_"+thisToggleFieldsArray[k].sName+"[]"
											 			)
											 			||
		...
		JFactory::getDocument()->addScriptDeclaration('
			...
			function bfCheckMaxlength(id, maxlength, showMaxlength){
				...
				if(showMaxlength){
					JQuery("#bfMaxLengthCounter"+id).text( "(" + (maxlength - JQuery("#ff_elem"+id).val().length) + " '.BFText::_('CHARS_LEFT').')" );
				}
			}
		...
		');
		// loading system css
		JFactory::getDocument()->addStyleSheet( JURI::root() . 'components/com_breezingforms/themes/quickmode/system.css' );
		
		$stylelink = '<!--[if IE 7]>' ."\n";
		$stylelink .= '<link rel="stylesheet" href="'.JURI::root() . 'components/com_breezingforms/themes/quickmode/system.ie7.css" />' ."\n";
		$stylelink .= '<![endif]-->' ."\n";
		JFactory::getDocument()->addCustomTag($stylelink);
		
		$stylelink = '<!--[if IE 6]>' ."\n";
		$stylelink .= '<link rel="stylesheet" href="'.JURI::root() . 'components/com_breezingforms/themes/quickmode/system.ie6.css" />' ."\n";
		$stylelink .= '<![endif]-->' ."\n";
		JFactory::getDocument()->addCustomTag($stylelink);
		
		$stylelink = '<!--[if IE]>' ."\n";
		$stylelink .= '<link rel="stylesheet" href="'.JURI::root() . 'components/com_breezingforms/themes/quickmode/system.ie.css" />' ."\n";
		$stylelink .= '<![endif]-->' ."\n";
		JFactory::getDocument()->addCustomTag($stylelink);
		
		// loading theme
		$this->cancelImagePath = JURI::root() . 'components/com_breezingforms/themes/quickmode/cancel.png';
		$this->uploadImagePath = JURI::root() . 'components/com_breezingforms/themes/quickmode/upload.png';
		if($this->rootMdata['theme'] != 'none' && @file_exists(JPATH_SITE .'/components/com_breezingforms/themes/quickmode/'. $this->rootMdata['theme'].'/theme.css')){
			JFactory::getDocument()->addStyleSheet( JURI::root() . 'components/com_breezingforms/themes/quickmode/'. $this->rootMdata['theme'].'/theme.css' );
			if(@file_exists(JPATH_SITE .'/components/com_breezingforms/themes/quickmode/'. $this->rootMdata['theme'].'/img/cancel.png')){
				$this->cancelImagePath = JURI::root() . 'components/com_breezingforms/themes/quickmode/'. $this->rootMdata['theme'].'/img/cancel.png';
			}
			if(@file_exists(JPATH_SITE .'/components/com_breezingforms/themes/quickmode/'. $this->rootMdata['theme'].'/img/upload.png')){
				$this->uploadImagePath = JURI::root() . 'components/com_breezingforms/themes/quickmode/'. $this->rootMdata['theme'].'/img/upload.png';
			}
		}
		...
		if(isset($dataObject['attributes']) && isset($dataObject['properties']) ){
			...
			if($mdata['type'] == 'page'){
				...
				if(isset($dataObject['properties']['name']) && isset($mdata['off']) && $mdata['off']){
					echo '<script>bfDeactivateSection.push("'.$dataObject['properties']['name'].'");</script>'."\n";
				}
				...
				if(!$mdata['hideLabel']){
					...
					if($mdata['bfType'] == 'bfTextarea' && isset($mdata['maxlength']) && $mdata['maxlength'] > 0 && isset($mdata['showMaxlengthCounter']) && $mdata['showMaxlengthCounter']){
						$maxlengthCounter = ' <span class=***bfMaxLengthCounter*** id=***bfMaxLengthCounter'.$mdata['dbId'].'***>('.$mdata['maxlength'].' '.BFText::_('CHARS_LEFT').')</span>';	
					}
					...
					if(trim($mdata['hint']) != ''){
						...
						$tipClose  = '</span>';
						$tipScript   = '<script>JQuery("#bfLabel'.$mdata['dbId'].'").qtip({ position: { adjust: { screen: true } }, content: "<b>'.addslashes(trim($mdata['label'])).'</b><br/>'.str_replace( array("\n","\r"), array("\\n",""), addslashes( trim($mdata['hint']) ) ).'", style: {tip: true, background: "#ffc", color: "#000000", border : { color: "#C0C0C0", width: 1 }, name: "cream" } })</script>';
					}
					
					echo '<label id="bfLabel'.$mdata['dbId'].'" for="ff_elem'.$mdata['dbId'].'">'.$tipOpen.str_replace("***","\"",$labelText).$tipClose.'</label>'.$tipScript."\n";
				}
				...
				switch($mdata['bfType']){
					...
					case 'bfTextarea':
						...
						echo '<textarea class="ff_elem" '.$onkeyup.$size.$tabIndex.$onclick.$onblur.$onchange.$onfocus.$onselect.$readonly.'name="ff_nm_'.$mdata['bfName'].'[]" id="ff_elem'.$mdata['dbId'].'">'.htmlentities(trim($mdata['value']), ENT_QUOTES, 'UTF-8').'</textarea>'."\n";
					...
					case 'bfRadioGroup':
						...
						if($mdata['group'] != ''){
							...
							$wrapClose = '';
							if(!$mdata['wrap']){
								 $wrapOpen = '<span class="bfElementGroupNoWrap">'."\n";
								 $wrapClose = '</span>'."\n";
							} else {
								$wrapOpen = '<span class="bfElementGroup">'."\n";
								$wrapClose = '</span>'."\n";
							}
					...
					case 'bfFile':
						...
						if(isset( $mdata['flashUploader'] ) && $mdata['flashUploader']){
							...
							var bfFlashFileQueue".$mdata['dbId']." = {};
									...
									'uploader'       : '".JURI::root()."components/com_breezingforms/libraries/jquery/uploadify.swf',
									'script'         : '".JURI::root()."index.php',
									...
									'onSelect'       : function(event, queueID, fileObj){
															...
															if(thebytes > 0 && fileObj.size > thebytes){
																...
																tooLarge = ' (".addslashes(BFText::_('FLASH_UPLOADER_TOO_LARGE')).")';
															}
									...
									'scriptData'     : { form: ".$this->p->form.", itemName : '".$mdata['bfName']."', bfFlashUploadTicket: '".$this->flashUploadTicket."', option: 'com_breezingforms', format: 'raw', flashUpload: 'true' },
					...
					case 'bfSummarize':
						...
						echo '<script>bfRegisterSummarize("ff_elem'.$mdata['dbId'].'", "'.$mdata['connectWith'].'", "'.$mdata['connectType'].'", "'.addslashes($mdata['emptyMessage']).'", '.($mdata['hideIfEmpty'] ? 'true' : 'false').')</script>';
						...
						break;
                                        ...
                                        case 'bfReCaptcha':
                                            ...
                                            if(isset($mdata['pubkey']) && $mdata['pubkey'] != ''){
                                                ...
                                                echo '<div id="bfReCaptchaDiv"></div>';
                                                echo '</span>'."\n";
                                            }
                                            else
                                            {
					...
					case 'bfCaptcha':
						...
						echo '<span class="bfCaptcha">'."\n";
						echo '<img id="ff_capimgValue" class="ff_capimg" src="'.JURI::root().'index.php?raw=true&option=com_breezingforms&bfCaptcha=true"/>'."\n";
						echo '<br/>';
						echo '<input class="ff_elem" type="text" name="bfCaptchaEntry" id="bfCaptchaEntry" />'."\n";
						echo '<a href="#" class="ff_elem" onclick="document.getElementById(\'bfCaptchaEntry\').value=\'\';document.getElementById(\'bfCaptchaEntry\').focus();document.getElementById(\'ff_capimgValue\').src = \''.JURI::root().'index.php?raw=true&option=com_breezingforms&bfCaptcha=true&\' + Math.random(); return false"><img alt="captcha" src="'.JURI::root().'components/com_breezingforms/images/captcha/refresh-captcha.png" border="0" /></a>'."\n";
						echo '</span>'."\n";
					...
					case 'bfCalendar':
						...
						echo '<span class="bfElementGroupNoWrap">'."\n";
				...
				if(isset($mdata['bfName']) && isset($mdata['off']) && $mdata['off']){
					echo '<script>bfDeactivateField["ff_nm_'.$mdata['bfName'].'[]"]=true;</script>'."\n";
				}
...
display:none;
		...
		if( $this->hasFlashUpload ){
			...
			JFactory::getDocument()->addScript(JURI::root() . 'components/com_breezingforms/libraries/jquery/swfobject.js');
			JFactory::getDocument()->addScript(JURI::root() . 'components/com_breezingforms/libraries/jquery/jquery.uploadify.v2.1.0.min.js');
			JFactory::getDocument()->addScript(JURI::root() . 'components/com_breezingforms/libraries/jquery/center.js');
			JFactory::getDocument()->addScriptDeclaration('
			...
			echo "<div style=\"visibility:hidden;\" id=\"bfSubmitMessage\">".BFText::_('SUBMIT_MESSAGE')."</div>";
		}
		echo '<script>if(document.getElementById("bfPage'.$this->p->page.'"))document.getElementById("bfPage'.$this->p->page.'").style.display = "";</script>'."\n";
