...
class facileFormsConfig extends facileFormsConf {
	...
	{
		if ($caller != 'index2.php' && $pkg != '') mosRedirect($caller."&pkg=$pkg");
		mosRedirect($caller);
	} // cancel
	...
	function save($option, $caller, $pkg)
	{
		...
		$this->store();
		if ($pkg != '') mosRedirect($caller."&pkg=$pkg", BFText::_('CONFIG_SAVED'));
		mosRedirect($caller, BFText::_('CONFIG_SAVED'));
	} // save
	...
	function instUploadPackage($option, $caller, $pkg)
	{
		...
		if ($uploadfile['name']=='') {
			HTML_facileFormsConf::message(
				$option, $caller, $pkg, BFText::_('INSTALLER_NOUPLOADFILE'), 'instpackage'
			);
			return;
		} // if
		$path = NULL;
	...
	function uploadFile( $filename, $userfile_name, &$path)
	{
		...
		if (!file_exists($baseDir))
			return BFText::_('INSTALLER_UPLOADNODIR');
		if (!is_writable($baseDir))
			return BFText::_('INSTALLER_UPLOADDIRNOTWRT');
		$path = $baseDir.'/'.$userfile_name;
		if (!move_uploaded_file($filename, $path))
			return BFText::_('INSTALLER_MOVEFAILED');
		$filemode = NULL;
		if (isset($mosConfig_fileperms)) {
			if ($mosConfig_fileperms!='')
				$filemode = octdec($mosConfig_fileperms);
		...
		if (isset($filemode))
			if (!@chmod($path, $filemode))
				return BFText::_('INSTALLER_CHMODFAILED');
		return '';
	} // uploadfile
	...
	function uninstPackages($option, $caller, $pkg, $ids)
	{
		...
		HTML_facileFormsConf::message(
			$option, $caller, $pkg,
			count($ids).' '.BFText::_('INSTALLER_PKGSUNINST'),
			'instpackage'
		);
	} // uninstPackages
