...
?>
	...
	function BF_QuickModeApp(){
					...
					for(var i = 0; i < target.children.length; i++){
						...
						if(mdata){
							if(target.children[i].attributes['class'] == 'bfQuickModePageClass'){
								...
								target.children[i].data.title = "<?php echo addslashes( BFText::_('PAGE') ) ?> " + (i+1);
		...
		*/
		...
		this.saveCheckboxProperties = function(mdata, item){
			// dynamic properties
			mdata.value = JQuery('#bfElementTypeCheckboxValue').val();
		...
		this.populateSummarizeProperties = function(mdata){
			...
			JQuery(option).val('');
			JQuery(option).text("<?php echo addslashes(BFText::_('CHOOSE_ONE')); ?>"); 
		...
		this.createTreeItem = function(obj){
				if(appScope.selectedTreeElement){
					switch(appScope.getNodeClass(appScope.selectedTreeElement)){
							...
							if(obj.attributes['class'] != 'bfQuickModePageClass'){
								...
								if(item){
						      		if(item.children){
						      			...
						      			alert("<?php echo addslashes(BFText::_('NO_CHILDREN_ERROR')); ?>");
						      		}
								}
							} else {
								alert("<?php echo addslashes(BFText::_('NEW_SECTION_ERROR')); ?>");
							}
						break;
						...
						break;
						default: alert("<?php echo addslashes(BFText::_('NEW_SECTION_ERROR')); ?>");
					}
			...
			if(mdata){
				...
				if(item){
					...
					if(mdata.lastPageThankYou && pagesSize > 1){
						...
						mdata.submittedScriptCondidtion = 0;
						mdata.submittedScriptCode = '';
					}
					item.properties = mdata;
				}
			}
					...
					if(target && target.children){
						if(target.attributes['class'] == 'bfQuickModeRootClass'){
							...
							if(target.properties.lastPageThankYou && pagesSize > 1){
								...
								target.properties.submittedScriptCondidtion = 0;
								target.properties.submittedScriptCode = '';
							}
						}
					}
			  		setTimeout("JQuery.tree_reference('bfElementExplorer').refresh()", 10); // give it time to close the context menu 
		...
		this.saveButton = function(){
			...
			if(appScope.selectedTreeElement){
				...
				switch( appScope.getNodeClass(appScope.selectedTreeElement) ) {
			  		case 'bfQuickModeRootClass':
			  			if(JQuery.trim(JQuery('#bfFormTitle').val()) == ''){
							alert("<?php echo addslashes(BFText::_('ERROR_ENTER_TITLE')) ?>");
							error = true;
						} 
						if(JQuery.trim(JQuery('#bfFormName').val()) == ''){
							alert("<?php echo addslashes(BFText::_('ERROR_ENTER_NAME')) ?>");
							error = true;
						}
						var myRegxp = /^([a-zA-Z0-9_]+)$/;
						if(!myRegxp.test(JQuery('#bfFormName').val())){
							alert("<?php echo addslashes(BFText::_('ERROR_ENTER_NAME_CHARACTERS')) ?>");
							error = true;
						}
						if(!error) {
					...
					break;
			  		...
			  		case 'bfQuickModeElementClass':
						if(JQuery.trim(JQuery('#bfElementLabel').val()) == ''){
							alert("<?php echo addslashes(BFText::_('ERROR_ENTER_LABEL')) ?>");
							error = true;
						} 
				...
				if(!error){
					...
					JQuery(".bfFadingMessage").html("<?php echo addslashes(BFText::_('SETTINGS_UPDATED')) ?>");
		...
		JQuery('#bfNewPageButton').click(
			function(){
				...
				if(item.properties.lastPageThankYou && pagesSize > 0){
					...
					item.properties.submittedScriptCondidtion = 0;
					item.properties.submittedScriptCode = '';
				}
				...
				var obj = {
				  ...
				  state: "open", 
				  data: { title: "<?php echo addslashes( BFText::_('PAGE') ) ?> " + pageNumber, icon: '<?php echo $iconBase . 'icon_page.png'?>'},
			      children : []
				};
				appScope.createTreeItem(obj);
				JQuery.tree_reference('bfElementExplorer').select_branch(JQuery('#'+id));
	...
	function createInitCode()
	{
		...
		if(mdata){
			...
			if (!confirm("<?php echo BFText::_('ELEMENTS_CREAINIT'); ?>\n<?php echo BFText::_('ELEMENTS_EXISTAPP'); ?>")) return;
			code =
			...
			if (oldcode != '')
				form.bfInitCode.value =
					code+
					"\n// -------------- <?php echo BFText::_('ELEMENTS_OLDBELOW'); ?> --------------\n\n"+
					oldcode;
			else
	...
	function createValidationCode()
	{
		...
		if(mdata){
			...
			if (!confirm("<?php echo BFText::_('ELEMENTS_CREAVALID'); ?>\n<?php echo BFText::_('ELEMENTS_EXISTAPP'); ?>")) return;
			code =
	...
	function createActionCode(element)
	{
		...
		if(mdata){
			...
			if (!confirm("<?php echo BFText::_('ELEMENTS_CREAACTION'); ?>\n<?php echo BFText::_('ELEMENTS_EXISTAPP'); ?>")) return;
			code =
	...
	function submitbutton(pressbutton)
	{
		...
		switch (pressbutton) {
			case 'save':
				...
				// raises the amount of allowed postdata if there are limits set
				// the experience showed that it stops around 65k data if there are webserver limitations, so we give triple the amount
				if(cVal.length > 60000){
					for( var i = 0; i < cVal.length; i++ ){
					 if(i < 60000){
					  ...
					  cVal3 += cVal[i]; // trying the rest of the data
					 }  
					}
				} else {
					cVal1 = cVal;
				...
				form.templateCode.value = cVal1;
				form.templateCode2.value = cVal2;
				form.templateCode3.value = cVal3;
			...
			case 'preview':
			    ...
			    SqueezeBox.loadModal("<?php echo JURI::root()?>index2.php?format=html&option=com_breezingforms&ff_form=<?php echo $formId ?>&ff_page=1","iframe",820,400);
				break; 
			case 'preview_site':
				SqueezeBox.initialize({});               
	...
	function loadFormSettings(){
		...
		SqueezeBox.loadModal("index3.php?option=com_breezingforms&task=editform&act=editpage&form=<?php echo $formId ?>&pkg=QuickModeForms","iframe",820,400);
	}
	...
	function loadEditor(){
		...
		SqueezeBox.loadModal("index3.php?option=com_breezingforms&act=quickmode_editor","iframe",640,380);
	}
	...
	<div style="float:left; margin-right: 3px;">
		<?php echo JToolBarHelper::custom('save', 'save.png', 'save_f2.png', BFText::_('TOOLBAR_QUICKMODE_SAVE'), false); ?>
		<?php
			
			if($formId != 0){
				echo JToolBarHelper::custom('preview', 'publish.png', 'save_f2.png', BFText::_('TOOLBAR_QUICKMODE_PREVIEW'), false);
				echo JToolBarHelper::custom('preview_site', 'publish.png', 'save_f2.png', BFText::_('SITE_PREVIEW'), false);
			}
		?>
		...
		<form action="index2.php" method="post" name="adminForm">
			...
			<input type="hidden" name="templateCode2" value="" />
			<input type="hidden" name="templateCode3" value="" />
...
<div id="bfQuickModeWrapper" class="bfClearfix">
	...
	<div style="float:left">
		<form onsubmit="return false;">
			<input id="bfNewPageButton" type="submit" value="<?php echo BFText::_('NEW_PAGE'); ?>"/>
			<input id="bfNewSectionButton" type="submit" value="<?php echo BFText::_('NEW_SECTION'); ?>"/>
			<input id="bfNewElementButton" type="submit" value="<?php echo BFText::_('NEW_ELEMENT'); ?>"/>
	...
	<div id="menutab" class="flora">
            <ul>
                <li><a onclick="JQuery('.bfFadingMessage').css('display','none')" href="#fragment-1"><span><div class="tab-items"><?php echo BFText::_('PROPERTIES') ?></div></span></a></li>
                <li><a onclick="JQuery('.bfFadingMessage').css('display','none')" href="#fragment-2"><span><div class="tab-element"><?php echo BFText::_('ADVANCED') ?></div></span></a></li>
            </ul>
			...
			<div class="m">
	            ...
	            <div id="fragment-1">
		            <div>
		            	...
		            	<input type="submit" value="<?php echo BFText::_('PROPERTIES_SAVE'); ?>" id="bfPropertySaveButtonTop"/>
		            	<!-- FORM PROPERTIES BEGIN -->
		            	<div class="bfProperties" id="bfFormProperties" style="display:none">
		            		<br/>
		            		<fieldset>
		            			<legend><?php echo BFText::_('FORM_PROPERTIES'); ?></legend>
		            			<label class="bfPropertyLabel" for="bfFormTitle"><?php echo BFText::_('FORM_TITLE'); ?></label>
		            			<input type="text" value="<?php echo htmlentities($formTitle,ENT_QUOTES,'UTF-8') ?>" id="bfFormTitle"/>
		            			<br/>
		            			<br/>
		            			<label class="bfPropertyLabel" for="bfFormName"><?php echo BFText::_('FORM_NAME'); ?></label>
		            			<input type="text" value="<?php echo htmlentities($formName,ENT_QUOTES,'UTF-8') ?>" id="bfFormName"/>
		            			<br/>
		            			<br/>
		            			<label class="bfPropertyLabel" for="bfFormDescription"><?php echo BFText::_('FORM_DESC'); ?></label>
		            			<textarea id="bfFormDescription"><?php echo htmlentities($formDesc,ENT_QUOTES,'UTF-8') ?></textarea>
		            			<br/>
			            		<br/>
			            		<label class="bfPropertyLabel" for="bfFormLastPageThankYou"><?php echo BFText::_('LAST_PAGE_THANK_YOU'); ?></label>
			            		<input type="checkbox" value="" id="bfFormLastPageThankYou"/>
		            			<hr/>
		            			<label class="bfPropertyLabel" for="bfFormMailNotification"><?php echo BFText::_('MAIL_NOTIFICATION'); ?></label>
		            			...
		            			<br/>
		            			<br/>
		            			<label class="bfPropertyLabel" for="bfFormMailRecipient"><?php echo BFText::_('MAIL_RECIPIENT'); ?></label>
		            			<input type="text" value="<?php echo htmlentities($formEmailadr,ENT_QUOTES,'UTF-8') ?>" id="bfFormMailRecipient"/>
		            			<hr/>
		            			<label class="bfPropertyLabel" for="bfSubmitIncludeYes"><?php echo BFText::_('FORM_SUBMIT_INCLUDE'); ?></label>
		            			<?php echo BFText::_('YES'); ?> <input checked="checked" type="radio" name="bfSubmitInclude" value="" id="bfSubmitIncludeYes"/>
		            			<?php echo BFText::_('NO'); ?> <input type="radio" name="bfSubmitInclude" value="" id="bfSubmitIncludeNo"/>
		            			<br/>
		            			<br/>
		            			<label class="bfPropertyLabel" for="bfFormSubmitLabel"><?php echo BFText::_('FORM_SUBMIT_LABEL'); ?></label>
		            			<input type="text" value="save" id="bfFormSubmitLabel"/>
		            			<hr/>
		            			<label class="bfPropertyLabel" for="bfPagingIncludeYes"><?php echo BFText::_('FORM_PAGING_INCLUDE'); ?></label>
		            			<?php echo BFText::_('YES'); ?> <input checked="checked" type="radio" name="bfPagingInclude" value="" id="bfPagingIncludeYes"/>
		            			<?php echo BFText::_('NO'); ?> <input type="radio" name="bfPagingInclude" value="" id="bfPagingIncludeNo"/>
		            			<br/>
		            			<br/>
		            			<label class="bfPropertyLabel" for="bfFormPagingNextLabel"><?php echo BFText::_('FORM_PAGING_NEXT_LABEL'); ?></label>
		            			<input type="text" value="next" id="bfFormPagingNextLabel"/>
		            			<br/>
		            			<br/>
		            			<label class="bfPropertyLabel" for="bfFormPagingPrevLabel"><?php echo BFText::_('FORM_PAGING_PREV_LABEL'); ?></label>
		            			<input type="text" value="back" id="bfFormPagingPrevLabel"/>
		            			<hr/>
		            			<label class="bfPropertyLabel" for="bfCancelIncludeYes"><?php echo BFText::_('FORM_CANCEL_INCLUDE'); ?></label>
		            			<?php echo BFText::_('YES'); ?> <input checked="checked" type="radio" name="bfCancelInclude" value="" id="bfCancelIncludeYes"/>
		            			<?php echo BFText::_('NO'); ?> <input type="radio" name="bfCancelInclude" value="" id="bfCancelIncludeNo"/>
		            			<br/>
		            			<br/>
		            			<label class="bfPropertyLabel" for="bfFormCancelLabel"><?php echo BFText::_('FORM_CANCEL_LABEL'); ?></label>
		            	...
		            	<div class="bfProperties" id="bfPageProperties" style="display:none">
		            		<br/>
		            		<fieldset>
		            		<legend><?php echo BFText::_('PAGE_PROPERTIES'); ?></legend>
		            		<label class="bfPropertyLabel" for="bfPageIntro"><?php echo BFText::_('PAGE_INTRO'); ?></label>
		            		<!-- <textarea id="bfPageIntro"></textarea>-->
		            		<a href="javascript:loadEditor()"><?php echo BFText::_('EDIT_INTRO'); ?></a>
		            		</fieldset>
		            	</div>
		            	<!-- PAGE PROPERTIES END -->
		            	...
		            	<div class="bfProperties" id="bfSectionProperties" style="display:none">
		            		<br/>
		            		<fieldset>
		            			<legend><?php echo BFText::_('SECTION_PROPERTIES'); ?></legend>
		            			<label class="bfPropertyLabel" for="bfSectionType"><?php echo BFText::_('SECTION_TYPE'); ?></label>
		            			<select id="bfSectionType">
		            				<option value="normal"><?php echo BFText::_('NORMAL'); ?></option>
		            				<option value="section"><?php echo BFText::_('FIELDSET'); ?></option>
		            			</select>
		            			<br/>
		            			<br/>
		            			<label class="bfPropertyLabel" for="bfSectionDisplayType"><?php echo BFText::_('SECTION_DISPLAY_TYPE'); ?></label>
		            			<select id="bfSectionDisplayType">
		            				<option value="inline"><?php echo BFText::_('INLINE'); ?></option>
		            				<option value="breaks"><?php echo BFText::_('BREAKS'); ?></option>
		            			</select>
		            			<br/>
		            			<br/>
		            			<label class="bfPropertyLabel" for="bfSectionTitle"><?php echo BFText::_('SECTION_TITLE'); ?></label>
		            			<input type="text" value="" id="bfSectionTitle"/>
		            			<br/>
		            			<br/>
		            			<label class="bfPropertyLabel" for="bfSectionName"><?php echo BFText::_('SECTION_NAME'); ?></label>
		            			<input type="text" value="" id="bfSectionName"/>
		            			<br/>
		            			<br/>
		            			<label class="bfPropertyLabel" for="bfSectionDescription"><?php echo BFText::_('SECTION_DESCRIPTION'); ?></label>
		            			<a href="javascript:loadEditor()"><?php echo BFText::_('EDIT_DESCRIPTION'); ?></a>
		            		</fieldset>
		            	</div>
		            	...
		            	<div class="bfProperties" id="bfElementProperties" style="display:none">
		            		<br/>
		            		<fieldset>
		            			<label class="bfPropertyLabel" for="bfElementType"><?php echo BFText::_('ELEMENT_TYPE'); ?></label>
		            			<select id="bfElementType">
		            				<option value=""><?php echo BFText::_('CHOOSE_ONE'); ?></option>
		            				<option value="bfElementTypeText"><?php echo BFText::_('TEXTFIELD'); ?></option>
		            				<option value="bfElementTypeTextarea"><?php echo BFText::_('TEXTAREA'); ?></option>
		            				<option value="bfElementTypeRadioGroup"><?php echo BFText::_('RADIO_GROUP'); ?></option>
		            				<option value="bfElementTypeCheckboxGroup"><?php echo BFText::_('CHECKBOX_GROUP'); ?></option>
		            				<option value="bfElementTypeCheckbox"><?php echo BFText::_('CHECKBOX'); ?></option>
		            				<option value="bfElementTypeSelect"><?php echo BFText::_('SELECT'); ?></option>
		            				<option value="bfElementTypeFile"><?php echo BFText::_('FILE'); ?></option>
		            				<option value="bfElementTypeSubmitButton"><?php echo BFText::_('SUBMIT_BUTTON'); ?></option>
		            				<option value="bfElementTypeHidden"><?php echo BFText::_('HIDDEN'); ?></option>
		            				<option value="bfElementTypeSummarize"><?php echo BFText::_('SUMMARIZE'); ?></option>
		            				<option value="bfElementTypeCaptcha"><?php echo BFText::_('CAPTCHA'); ?></option>
                                                        <option value="bfElementTypeReCaptcha"><?php echo BFText::_('ReCaptcha'); ?></option>
		            				<option value="bfElementTypeCalendar"><?php echo BFText::_('CALENDAR'); ?></option>
		            				<option value="bfElementTypePayPal"><?php echo BFText::_('PAYPAL'); ?></option>
		            				<option value="bfElementTypeSofortueberweisung"><?php echo BFText::_('SOFORTUEBERWEISUNG'); ?></option>
		            			</select>
		            			<br/>
		            			<br/>
		            			<legend><?php echo BFText::_('ELEMENT_PROPERTIES'); ?></legend>
		            			<label class="bfPropertyLabel" for="bfElementLabel"><?php echo BFText::_('ELEMENT_LABEL'); ?></label>
		            			<input type="text" value="" id="bfElementLabel"/>
		            			<br/>
		            			<br/>
		            			<label class="bfPropertyLabel" for="bfElementName"><?php echo BFText::_('ELEMENT_NAME'); ?></label>
			            		<input type="text" value="" id="bfElementName"/>
			            		<!-- HIDDEN BEGIN -->
		            			<div class="bfElementTypeClass" id="bfElementTypeHidden" style="display:none">
		            				<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeHiddenValue"><?php echo BFText::_('ELEMENT_VALUE'); ?></label>
		            			...
		            			<div class="bfElementTypeClass" id="bfElementTypeSummarize" style="display:none">
		            				<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeSummarizeConnectWith"><?php echo BFText::_('ELEMENT_SUMMARIZE_CONNECT_WITH'); ?></label>
			            			<select id="bfElementTypeSummarizeConnectWith">
		            					<option value=""><?php echo BFText::_('CHOOSE_ONE'); ?></option>
		            				</select>
		            				<br/>
		            				<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeSummarizeEmptyMessage"><?php echo BFText::_('ELEMENT_EMPTY_MESSAGE'); ?></label>
			            			<input type="text" value="" id="bfElementTypeSummarizeEmptyMessage"/>
			            			<br/>
			            			<br/>
		            				<label class="bfPropertyLabel" for="bfElementTypeSummarizeHideIfEmpty"><?php echo BFText::_('ELEMENT_HIDE_EMPTY'); ?></label>
				            		<input type="checkbox" value="" id="bfElementTypeSummarizeHideIfEmpty"/>
		            				<br/>
			            			<br/>
		            				<label class="bfPropertyLabel" for="bfElementTypeSummarizeUseElementLabel"><?php echo BFText::_('ELEMENT_USE_LABEL'); ?></label>
		            			...
		            			<div class="bfElementTypeClass" id="bfElementTypeText" style="display:none">
		            				<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeTextValue"><?php echo BFText::_('ELEMENT_VALUE'); ?></label>
			            			<input type="text" value="" id="bfElementTypeTextValue"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeTextSize"><?php echo BFText::_('ELEMENT_SIZE'); ?></label>
			            			<input type="text" value="" id="bfElementTypeTextSize"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeTextMaxLength"><?php echo BFText::_('ELEMENT_MAX_LENGTH'); ?></label>
			            			<input type="text" value="" id="bfElementTypeTextMaxLength"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeTextHint"><?php echo BFText::_('ELEMENT_HINT'); ?></label>
		            			...
		            			<div class="bfElementTypeClass" id="bfElementTypeTextarea" style="display:none">
		            				<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeTextareaValue"><?php echo BFText::_('ELEMENT_VALUE'); ?></label>
			            			<textarea id="bfElementTypeTextareaValue"></textarea>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeTextareaWidth"><?php echo BFText::_('ELEMENT_WIDTH'); ?></label>
			            			<input type="text" value="" id="bfElementTypeTextareaWidth"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeTextareaHeight"><?php echo BFText::_('ELEMENT_HEIGHT'); ?></label>
			            			<input type="text" value="" id="bfElementTypeTextareaHeight"/>
			            			<br/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeTextareaMaxLength"><?php echo BFText::_('ELEMENT_MAX_LENGTH'); ?></label>
			            			<input type="text" value="" id="bfElementTypeTextareaMaxLength"/>
			            			<br/>
				            		<br/>
				            		<label class="bfPropertyLabel" for=bfElementTypeTextareaMaxLengthShow><?php echo BFText::_('ELEMENT_MAX_LENGTH_SHOW'); ?></label>
				            		<input type="checkbox" value="" id="bfElementTypeTextareaMaxLengthShow"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeTextareaHint"><?php echo BFText::_('ELEMENT_HINT'); ?></label>
		            			...
		            			<div class="bfElementTypeClass" id="bfElementTypeRadioGroup" style="display:none">
		            				<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeRadioGroupGroups"><?php echo BFText::_('ELEMENT_GROUP'); ?></label>
			            			<textarea id="bfElementTypeRadioGroupGroups"></textarea>
				            		<br/>
				            		<br/>
				            		<label class="bfPropertyLabel" for="bfElementTypeRadioGroupReadonly"><?php echo BFText::_('ELEMENT_READONLY'); ?></label>
				            		<input type="checkbox" value="" id="bfElementTypeRadioGroupReadonly"/>
				            		<br/>
				            		<br/>
				            		<label class="bfPropertyLabel" for="bfElementTypeRadioGroupWrap"><?php echo BFText::_('ELEMENT_WRAP'); ?></label>
				            		<input type="checkbox" value="" id="bfElementTypeRadioGroupWrap"/>
				            		<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeRadioGroupHint"><?php echo BFText::_('ELEMENT_HINT'); ?></label>
		            			...
		            			<div class="bfElementTypeClass" id="bfElementTypeSubmitButton" style="display:none">
		            				<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeSubmitButtonValue"><?php echo BFText::_('ELEMENT_VALUE'); ?></label>
			            			<input type="text" value="" id="bfElementTypeSubmitButtonValue"/>
				            		<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeSubmitButtonHint"><?php echo BFText::_('ELEMENT_HINT'); ?></label>
		            			...
		            			<div class="bfElementTypeClass" id="bfElementTypePayPal" style="display:none">
		            				<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypePayPalBusiness"><?php echo BFText::_('ELEMENT_BUSINESS'); ?></label>
			            			<input type="text" value="" id="bfElementTypePayPalBusiness"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypePayPalToken"><?php echo BFText::_('ELEMENT_TOKEN'); ?></label>
			            			<input type="text" value="" id="bfElementTypePayPalToken"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypePayPalItemname"><?php echo BFText::_('ELEMENT_ITEMNAME'); ?></label>
			            			<input type="text" value="" id="bfElementTypePayPalItemname"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypePayPalItemnumber"><?php echo BFText::_('ELEMENT_ITEMNUMBER'); ?></label>
			            			<input type="text" value="" id="bfElementTypePayPalItemnumber"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypePayPalAmount"><?php echo BFText::_('ELEMENT_AMOUNT'); ?></label>
			            			<input type="text" value="" id="bfElementTypePayPalAmount"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypePayPalTax"><?php echo BFText::_('ELEMENT_TAX'); ?></label>
			            			<input type="text" value="" id="bfElementTypePayPalTax"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypePayPalThankYouPage"><?php echo BFText::_('ELEMENT_THANKYOU_PAGE'); ?></label>
			            			<input type="text" value="" id="bfElementTypePayPalThankYouPage"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypePayPalLocale"><?php echo BFText::_('ELEMENT_LOCALE'); ?></label>
			            			<input type="text" value="" id="bfElementTypePayPalLocale"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypePayPalCurrencyCode"><?php echo BFText::_('ELEMENT_CURRENCY_CODE'); ?></label>
			            			<input type="text" value="" id="bfElementTypePayPalCurrencyCode"/>
			            			<br/>
				            		<br/>
				            		<label class="bfPropertyLabel" for=bfElementTypePayPalSendNotificationAfterPayment><?php echo BFText::_('NOTIFICATION_AFTER_PAYMENT'); ?></label>
				            		<input type="checkbox" value="" id="bfElementTypePayPalSendNotificationAfterPayment"/>
			            			
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypePayPalHint"><?php echo BFText::_('ELEMENT_HINT'); ?></label>
		            			...
		            			<div class="bfElementTypeClass" id="bfElementTypeSofortueberweisung" style="display:none">
		            				<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeSofortueberweisungUserId"><?php echo BFText::_('ELEMENT_USERID'); ?></label>
			            			<input type="text" value="" id="bfElementTypeSofortueberweisungUserId"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeSofortueberweisungProjectId"><?php echo BFText::_('ELEMENT_PROJECTID'); ?></label>
			            			<input type="text" value="" id="bfElementTypeSofortueberweisungProjectId"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeSofortueberweisungProjectPassword"><?php echo BFText::_('ELEMENT_PROJECT_PASSWORD'); ?></label>
			            			<input type="password" value="" id="bfElementTypeSofortueberweisungProjectPassword"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeSofortueberweisungReason1"><?php echo BFText::_('ELEMENT_REASON1'); ?></label>
			            			<input type="text" value="" id="bfElementTypeSofortueberweisungReason1"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeSofortueberweisungReason2"><?php echo BFText::_('ELEMENT_REASON2'); ?></label>
			            			<input type="text" value="" id="bfElementTypeSofortueberweisungReason2"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeSofortueberweisungAmount"><?php echo BFText::_('ELEMENT_AMOUNT'); ?></label>
			            			<input type="text" value="" id="bfElementTypeSofortueberweisungAmount"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeSofortueberweisungThankYouPage"><?php echo BFText::_('ELEMENT_THANKYOU_PAGE'); ?></label>
			            			<input type="text" value="" id="bfElementTypeSofortueberweisungThankYouPage"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeSofortueberweisungLanguageId"><?php echo BFText::_('ELEMENT_LANGUAGE_ID'); ?></label>
			            			<input type="text" value="" id="bfElementTypeSofortueberweisungLanguageId"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeSofortueberweisungCurrencyId"><?php echo BFText::_('ELEMENT_CURRENCY_ID'); ?></label>
			            			<input type="text" value="" id="bfElementTypeSofortueberweisungCurrencyId"/>
			            			<br/>
				            		<br/>
				            		<label class="bfPropertyLabel" for=bfElementTypeSofortueberweisungMailback><?php echo BFText::_('ELEMENT_MAILBACK'); ?></label>
				            		<input type="checkbox" value="" id="bfElementTypeSofortueberweisungMailback"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeSofortueberweisungHint"><?php echo BFText::_('ELEMENT_HINT'); ?></label>
		            			...
		            			<div class="bfElementTypeClass" id="bfElementTypeCaptcha" style="display:none">
		            				<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeCaptchaHint"><?php echo BFText::_('ELEMENT_HINT'); ?></label>
		            			...
		            			<div class="bfElementTypeClass" id="bfElementTypeReCaptcha" style="display:none">
		            				<br/>
                                                        ...
                                                        <label class="bfPropertyLabel" for=bfElementTypeReCaptchaPubkey><?php echo BFText::_('Public Key'); ?></label>
				            		<input type="text" value="" id="bfElementTypeReCaptchaPubkey"/>
                                                        <br/>
		            				<br/>
                                                        <label class="bfPropertyLabel" for=bfElementTypeReCaptchaPrivkey><?php echo BFText::_('Private Key'); ?></label>
				            		<input type="text" value="" id="bfElementTypeReCaptchaPrivkey"/>
                                                        <br/>
		            				<br/>
                                                        <label class="bfPropertyLabel" for=bfElementTypeReCaptchaTheme><?php echo BFText::_('Theme'); ?></label>
				            		<input type="text" value="red" id="bfElementTypeReCaptchaTheme"/> ('red', 'white', 'blackglass', 'clean', 'custom')
                                                        <br/>
		            				<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeReCaptchaHint"><?php echo BFText::_('ELEMENT_HINT'); ?></label>
		            			...
		            			<div class="bfElementTypeClass" id="bfElementTypeCalendar" style="display:none">
		            				<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeCalendarFormat"><?php echo BFText::_('ELEMENT_FORMAT'); ?></label>
			            			<input type="text" value="" id="bfElementTypeCalendarFormat"/>
		            				<br/>
		            				<br/>
		            				<label class="bfPropertyLabel" for="bfElementTypeCalendarValue"><?php echo BFText::_('ELEMENT_VALUE'); ?></label>
			            			<input type="text" value="" id="bfElementTypeCalendarValue"/>
			            			<br/>
		            				<br/>
		            				<label class="bfPropertyLabel" for="bfElementTypeCalendarSize"><?php echo BFText::_('ELEMENT_SIZE'); ?></label>
			            			<input type="text" value="" id="bfElementTypeCalendarSize"/>
			            			<br/>
		            				<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeCalendarHint"><?php echo BFText::_('ELEMENT_HINT'); ?></label>
		            			...
		            			<div class="bfElementTypeClass" id="bfElementTypeCheckboxGroup" style="display:none">
		            				<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeCheckboxGroupGroups"><?php echo BFText::_('ELEMENT_GROUP'); ?></label>
			            			<textarea id="bfElementTypeCheckboxGroupGroups"></textarea>
				            		<br/>
				            		<br/>
				            		<label class="bfPropertyLabel" for="bfElementTypeCheckboxGroupReadonly"><?php echo BFText::_('ELEMENT_READONLY'); ?></label>
				            		<input type="checkbox" value="" id="bfElementTypeCheckboxGroupReadonly"/>
				            		<br/>
				            		<br/>
				            		<label class="bfPropertyLabel" for="bfElementTypeCheckboxGroupWrap"><?php echo BFText::_('ELEMENT_WRAP'); ?></label>
				            		<input type="checkbox" value="" id="bfElementTypeCheckboxGroupWrap"/>
				            		<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeCheckboxGroupHint"><?php echo BFText::_('ELEMENT_HINT'); ?></label>
		            			...
		            			<div class="bfElementTypeClass" id="bfElementTypeCheckbox" style="display:none">
		            				<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeCheckboxValue"><?php echo BFText::_('ELEMENT_VALUE'); ?></label>
			            			<textarea id="bfElementTypeCheckboxValue"></textarea>
			            			<br/>
				            		<br/>
				            		<label class="bfPropertyLabel" for="bfElementTypeCheckboxChecked"><?php echo BFText::_('ELEMENT_CHECKED'); ?></label>
				            		<input type="checkbox" value="" id="bfElementTypeCheckboxChecked"/>
				            		<br/>
				            		<br/>
				            		<label class="bfPropertyLabel" for="bfElementTypeCheckboxReadonly"><?php echo BFText::_('ELEMENT_READONLY'); ?></label>
				            		<input type="checkbox" value="" id="bfElementTypeCheckboxReadonly"/>
				            		<br/>
				            		<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeCheckboxHint"><?php echo BFText::_('ELEMENT_HINT'); ?></label>
		            			...
		            			<div class="bfElementTypeClass" id="bfElementTypeSelect" style="display:none">
		            				<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeSelectList"><?php echo BFText::_('ELEMENT_LIST'); ?></label>
			            			<textarea id="bfElementTypeSelectList"></textarea>
			            			<br/>
				            		<br/>
				            		<label class="bfPropertyLabel" for="bfElementTypeSelectMultiple"><?php echo BFText::_('ELEMENT_MULTIPLE'); ?></label>
				            		<input type="checkbox" value="" id="bfElementTypeSelectMultiple"/>
				            		<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeSelectListWidth"><?php echo BFText::_('ELEMENT_WIDTH'); ?></label>
			            			<input type="text" value="" id="bfElementTypeSelectListWidth"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeSelectListHeight"><?php echo BFText::_('ELEMENT_HEIGHT'); ?></label>
			            			<input type="text" value="" id="bfElementTypeSelectListHeight"/>
				            		<br/>
				            		<br/>
				            		<label class="bfPropertyLabel" for="bfElementTypeSelectReadonly"><?php echo BFText::_('ELEMENT_READONLY'); ?></label>
				            		<input type="checkbox" value="" id="bfElementTypeSelectReadonly"/>
				            		<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeSelectHint"><?php echo BFText::_('ELEMENT_HINT'); ?></label>
		            			...
		            			<div class="bfElementTypeClass" id="bfElementTypeFile" style="display:none">
		            				<br/>
				            		<label class="bfPropertyLabel" for="bfElementTypeFileReadonly"><?php echo BFText::_('ELEMENT_READONLY'); ?></label>
				            		<input type="checkbox" value="" id="bfElementTypeFileReadonly"/>
				            		<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementTypeFileHint"><?php echo BFText::_('ELEMENT_HINT'); ?></label>
			            			<textarea id="bfElementTypeFileHint"></textarea>
		            			</div>
		            			<!-- FILE END -->
		            		</fieldset>
		            		<fieldset id="bfValidationScript" style="display:none">
		            			<legend><?php echo BFText::_('ELEMENT_VALIDATION'); ?></legend>
		            			<span id="bfElementValidationRequiredSet" style="display:none">
			            			<label class="bfPropertyLabel" for="bfElementValidationRequired"><?php echo BFText::_('ELEMENT_VALIDATION_REQUIRED'); ?></label>
				            		<input type="checkbox" value="" id="bfElementValidationRequired"/>
				            		<br/>
				            		<br/>
			            		</span>
		            			<label class="bfPropertyLabel" for="bfElementValidation"><?php echo BFText::_('ELEMENT_VALIDATION_LABEL'); ?></label>
		            			<div>
									...
									<div id="bfValidationScriptFlags" style="display:none">
										<hr/>
										<?php echo BFText::_('ERROR_MESSAGE') ?>: <input type="text" style="width:100%" maxlength="255" class="bfValidationMessage" id="bfValidationMessage" name="bfValidationMessage" value="" class="inputbox"/>
									</div>
									...
									<div id="bfValidationScriptCustom" style="display:none">
										<hr/>
										<div style="cursor: pointer;" onclick="createValidationCode()"><?php echo BFText::_('CREATE_CODE_FRAMEWORK') ?></div>
		            	...
		            	<div class="bfFadingMessage" style="display:none"></div>
		            	<input type="submit" value="<?php echo BFText::_('PROPERTIES_SAVE'); ?>" id="bfPropertySaveButton"/>
		            </div>
	            </div>
	            ...
	            <div id="fragment-2">
	            	<div>
	            		<div class="bfFadingMessage" style="display:none"></div>
			            <input type="submit" value="<?php echo BFText::_('PROPERTIES_SAVE'); ?>" id="bfAdvancedSaveButtonTop"/>
			            <div class="bfAdvanced" id="bfPageAdvanced" style="display:none">
		            	</div>
		            	<div class="bfAdvanced" id="bfFormAdvanced" style="display:none">
		            		<br/>
			            	<fieldset>
			            		<legend><?php echo BFText::_('ADVANCED_FORM_OPTIONS'); ?></legend>
			            		<?php if($formId != 0){ ?>
			            		<a href="javascript:loadFormSettings()"><?php echo htmlentities( BFText::_('MORE_OPTIONS'), ENT_QUOTES, 'UTF-8' ) ?></a>
			            		<?php } ?>
			            	</fieldset>
			            	<fieldset>
			            		<legend><?php echo BFText::_('ADVANCED_FORM_OTHER'); ?></legend>
			            		<label class="bfPropertyLabel" for="bfTheme"><?php echo BFText::_('THEME'); ?></label>
			            		<select id="bfTheme">
			            		<?php
								$tCount = count($themes);
								for($i = 0; $i < $tCount; $i++){
			            		...
			            		<br/>
			            		<br/>
			            		<label class="bfPropertyLabel" for="bfElementAdvancedUseErrorAlerts"><?php echo BFText::_('USE_ERROR_ALERTS'); ?></label>
			            		<input type="checkbox" value="" id="bfElementAdvancedUseErrorAlerts"/>
                                                <br/>
			            		<br/>
                                                <label class="bfPropertyLabel" for="bfElementAdvancedUseDefaultErrors"><?php echo BFText::_('IF_NOT_USE_ERROR_ALERTS'); ?></label>
			            		<?php echo BFText::_('IF_USE_DEFAULT_ERRROS'); ?> <input type="checkbox" value="" id="bfElementAdvancedUseDefaultErrors"/>
                                                <?php echo BFText::_('IF_USE_BALLOON_ERRORS'); ?> <input type="checkbox" value="" id="bfElementAdvancedUseBalloonErrors"/>
			            		<br/>
			            		<br/>
			            		<label class="bfPropertyLabel" for="bfElementAdvancedFadeIn"><?php echo BFText::_('FADE_IN'); ?></label>
			            		<input type="checkbox" value="" id="bfElementAdvancedFadeIn"/>
			            		<br/>
			            		<br/>
			            		<label class="bfPropertyLabel" for="bfElementAdvancedRollover"><?php echo BFText::_('ROLLOVER'); ?></label>
			            		<input type="checkbox" value="" id="bfElementAdvancedRollover"/>
			            		<br/>
			            		<br/>
			            		<label class="bfPropertyLabel" for="bfElementAdvancedRolloverColor"><?php echo BFText::_('ROLLOVER_COLOR'); ?></label>
			            		<input type="text" value="" id="bfElementAdvancedRolloverColor"/>
			            		<br/>
		            			<br/>
		            			<label class="bfPropertyLabel" for="bfElementAdvancedToggleFields"><?php echo BFText::_('FORM_TOGGLEFIELDS'); ?></label>
		            			<textarea id="bfElementAdvancedToggleFields"></textarea>
			            	</fieldset>
			            </div>
			            <div class="bfAdvanced" id="bfSectionAdvanced" style="display:none">
			            	<br/>
			            	<label class="bfPropertyLabel" for="bfSectionAdvancedTurnOff"><?php echo BFText::_('TURN_OFF_INITIALLY'); ?></label>
			            	<input type="checkbox" value="" id="bfSectionAdvancedTurnOff"/>
			            </div>
			            <div class="bfAdvanced" id="bfElementAdvanced" style="display:none">
			            	<br/>
			            	<fieldset>
			            		<legend><?php echo BFText::_('ADVANCED_ELEMENT_OPTIONS'); ?></legend>
			            		<!-- HIDDEN BEGIN -->
			            		<div class="bfElementTypeClass" id="bfElementTypeHiddenAdvanced" style="display:none">
			            			<label class="bfPropertyLabel" for="bfElementHiddenAdvancedOrderNumber"><?php echo BFText::_('ORDER_NUMBER'); ?></label>
			            			<input type="text" value="" id="bfElementHiddenAdvancedOrderNumber"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementHiddenAdvancedLogging"><?php echo BFText::_('LOGGING'); ?></label>
			            		...
			            		<div class="bfElementTypeClass" id="bfElementTypeSummarizeAdvanced" style="display:none">
			            			<label class="bfPropertyLabel" for="bfElementAdvancedSummarizeCalc"><?php echo BFText::_('ELEMENT_CALC'); ?></label>
			            		...
			            		<div class="bfElementTypeClass" id="bfElementTypeTextAdvanced" style="display:none">
				            		<label class="bfPropertyLabel" for="bfElementAdvancedPassword"><?php echo BFText::_('PASSWORD'); ?></label>
			            			<input type="checkbox" value="" id="bfElementAdvancedPassword"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementAdvancedReadOnly"><?php echo BFText::_('READONLY'); ?></label>
			            			<input type="checkbox" value="" id="bfElementAdvancedReadOnly"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementAdvancedMailback"><?php echo BFText::_('MAILBACK'); ?></label>
			            			<input type="checkbox" value="" id="bfElementAdvancedMailback"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementAdvancedMailbackAsSender"><?php echo BFText::_('MAILBACK_AS_SENDER'); ?></label>
			            			<input type="checkbox" value="" id="bfElementAdvancedMailbackAsSender"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementAdvancedMailbackfile"><?php echo BFText::_('MAILBACKFILE'); ?></label>
			            			<input type="text" value="" id="bfElementAdvancedMailbackfile"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementAdvancedHideLabel"><?php echo BFText::_('HIDE_LABEL'); ?></label>
			            			<input checked="checked" type="checkbox" value="" id="bfElementAdvancedHideLabel"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementAdvancedLogging"><?php echo BFText::_('LOGGING'); ?></label>
			            			<input checked="checked" type="checkbox" value="" id="bfElementAdvancedLogging"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementOrderNumber"><?php echo BFText::_('ORDER_NUMBER'); ?></label>
			            		...
			            		<div class="bfElementTypeClass" id="bfElementTypeTextareaAdvanced" style="display:none">
			            			<label class="bfPropertyLabel" for="bfElementTextareaAdvancedHideLabel"><?php echo BFText::_('HIDE_LABEL'); ?></label>
			            			<input checked="checked" type="checkbox" value="" id="bfElementTextareaAdvancedHideLabel"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementTextareaAdvancedLogging"><?php echo BFText::_('LOGGING'); ?></label>
			            			<input checked="checked" type="checkbox" value="" id="bfElementTextareaAdvancedLogging"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementTextareaAdvancedOrderNumber"><?php echo BFText::_('ORDER_NUMBER'); ?></label>
			            		...
			            		<div class="bfElementTypeClass" id="bfElementTypeRadioGroupAdvanced" style="display:none">
			            			<label class="bfPropertyLabel" for="bfElementRadioGroupAdvancedHideLabel"><?php echo BFText::_('HIDE_LABEL'); ?></label>
			            			<input checked="checked" type="checkbox" value="" id="bfElementRadioGroupAdvancedHideLabel"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementRadioGroupAdvancedLogging"><?php echo BFText::_('LOGGING'); ?></label>
			            			<input checked="checked" type="checkbox" value="" id="bfElementRadioGroupAdvancedLogging"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementRadioGroupAdvancedOrderNumber"><?php echo BFText::_('ORDER_NUMBER'); ?></label>
			            		...
			            		<div class="bfElementTypeClass" id="bfElementTypeSubmitButtonAdvanced" style="display:none">
			            			<label class="bfPropertyLabel" for="bfElementSubmitButtonAdvancedHideLabel"><?php echo BFText::_('HIDE_LABEL'); ?></label>
			            			<input checked="checked" type="checkbox" value="" id="bfElementSubmitButtonAdvancedHideLabel"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementSubmitButtonAdvancedSrc"><?php echo BFText::_('SOURCE'); ?></label>
			            		...
			            		<div class="bfElementTypeClass" id="bfElementTypePayPalAdvanced" style="display:none">
			            			<label class="bfPropertyLabel" for="bfElementPayPalAdvancedHideLabel"><?php echo BFText::_('HIDE_LABEL'); ?></label>
			            			<input checked="checked" type="checkbox" value="" id="bfElementPayPalAdvancedHideLabel"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementPayPalAdvancedImage"><?php echo BFText::_('IMAGE'); ?></label>
			            			<input type="text" value="" id="bfElementPayPalAdvancedImage"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementPayPalAdvancedTestaccount"><?php echo BFText::_('TESTACCOUNT'); ?></label>
			            			<input checked="checked" type="checkbox" value="" id="bfElementPayPalAdvancedTestaccount"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementPayPalAdvancedTestBusiness"><?php echo BFText::_('TESTBUSINESS'); ?></label>
			            			<input type="text" value="" id="bfElementPayPalAdvancedTestBusiness"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementPayPalAdvancedTestToken"><?php echo BFText::_('TESTTOKEN'); ?></label>
			            			<input type="text" value="" id="bfElementPayPalAdvancedTestToken"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementPayPalAdvancedDownloadableFile"><?php echo BFText::_('DOWNLOADABLE_FILE'); ?></label>
			            			<input checked="checked" type="checkbox" value="" id="bfElementPayPalAdvancedDownloadableFile"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementPayPalAdvancedFilepath"><?php echo BFText::_('FILEPATH'); ?></label>
			            			<input type="text" value="" id="bfElementPayPalAdvancedFilepath"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementPayPalAdvancedDownloadTries"><?php echo BFText::_('DOWNLOAD_TRIES'); ?></label>
			            		...
			            		<div class="bfElementTypeClass" id="bfElementTypeSofortueberweisungAdvanced" style="display:none">
			            			<label class="bfPropertyLabel" for="bfElementSofortueberweisungAdvancedHideLabel"><?php echo BFText::_('HIDE_LABEL'); ?></label>
			            			<input checked="checked" type="checkbox" value="" id="bfElementSofortueberweisungAdvancedHideLabel"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementSofortueberweisungAdvancedImage"><?php echo BFText::_('IMAGE'); ?></label>
			            			<input type="text" value="" id="bfElementSofortueberweisungAdvancedImage"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementSofortueberweisungAdvancedDownloadableFile"><?php echo BFText::_('DOWNLOADABLE_FILE'); ?></label>
			            			<input checked="checked" type="checkbox" value="" id="bfElementSofortueberweisungAdvancedDownloadableFile"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementSofortueberweisungAdvancedFilepath"><?php echo BFText::_('FILEPATH'); ?></label>
			            			<input type="text" value="" id="bfElementSofortueberweisungAdvancedFilepath"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementSofortueberweisungAdvancedDownloadTries"><?php echo BFText::_('DOWNLOAD_TRIES'); ?></label>
			            		...
			            		<div class="bfElementTypeClass" id="bfElementTypeCaptchaAdvanced" style="display:none">
			            			<label class="bfPropertyLabel" for="bfElementCaptchaAdvancedHideLabel"><?php echo BFText::_('HIDE_LABEL'); ?></label>
			            		...
			            		<div class="bfElementTypeClass" id="bfElementTypeReCaptchaAdvanced" style="display:none">
			            			<label class="bfPropertyLabel" for="bfElementReCaptchaAdvancedHideLabel"><?php echo BFText::_('HIDE_LABEL'); ?></label>
			            		...
			            		<div class="bfElementTypeClass" id="bfElementTypeCalendarAdvanced" style="display:none">
			            			<label class="bfPropertyLabel" for="bfElementCalendarAdvancedHideLabel"><?php echo BFText::_('HIDE_LABEL'); ?></label>
			            		...
			            		<div class="bfElementTypeClass" id="bfElementTypeCheckboxGroupAdvanced" style="display:none">
			            			<label class="bfPropertyLabel" for="bfElementCheckboxGroupAdvancedHideLabel"><?php echo BFText::_('HIDE_LABEL'); ?></label>
			            			<input checked="checked" type="checkbox" value="" id="bfElementCheckboxGroupAdvancedHideLabel"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementCheckboxGroupAdvancedLogging"><?php echo BFText::_('LOGGING'); ?></label>
			            			<input checked="checked" type="checkbox" value="" id="bfElementCheckboxGroupAdvancedLogging"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementCheckboxGroupAdvancedOrderNumber"><?php echo BFText::_('ORDER_NUMBER'); ?></label>
			            		...
			            		<div class="bfElementTypeClass" id="bfElementTypeCheckboxAdvanced" style="display:none">
			            			<label class="bfPropertyLabel" for="bfElementCheckboxAdvancedMailbackAccept"><?php echo BFText::_('MAILBACK_ACCEPT'); ?></label>
			            			<input checked="checked" type="checkbox" value="" id="bfElementCheckboxAdvancedMailbackAccept"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementCheckboxAdvancedMailbackConnectWith"><?php echo BFText::_('MAILBACK_CONNECT_WITH'); ?></label>
			            			<input type="text" value="" id="bfElementCheckboxAdvancedMailbackConnectWith"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementCheckboxAdvancedHideLabel"><?php echo BFText::_('HIDE_LABEL'); ?></label>
			            			<input checked="checked" type="checkbox" value="" id="bfElementCheckboxAdvancedHideLabel"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementCheckboxAdvancedLogging"><?php echo BFText::_('LOGGING'); ?></label>
			            			<input checked="checked" type="checkbox" value="" id="bfElementCheckboxAdvancedLogging"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementCheckboxAdvancedOrderNumber"><?php echo BFText::_('ORDER_NUMBER'); ?></label>
			            		...
			            		<div class="bfElementTypeClass" id="bfElementTypeSelectAdvanced" style="display:none">
			            			<label class="bfPropertyLabel" for="bfElementSelectAdvancedHideLabel"><?php echo BFText::_('HIDE_LABEL'); ?></label>
			            			<input checked="checked" type="checkbox" value="" id="bfElementSelectAdvancedHideLabel"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementSelectAdvancedMailback"><?php echo BFText::_('MAILBACK'); ?></label>
			            			<input checked="checked" type="checkbox" value="" id="bfElementSelectAdvancedMailback"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementSelectAdvancedLogging"><?php echo BFText::_('LOGGING'); ?></label>
			            			<input checked="checked" type="checkbox" value="" id="bfElementSelectAdvancedLogging"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementSelectAdvancedOrderNumber"><?php echo BFText::_('ORDER_NUMBER'); ?></label>
			            		...
			            		<div class="bfElementTypeClass" id="bfElementTypeFileAdvanced" style="display:none">
			            			<label class="bfPropertyLabel" for="bfElementFileAdvancedFlashUploader"><?php echo BFText::_('FLASH_UPLOADER'); ?></label>
			            			<input type="checkbox" value="" id="bfElementFileAdvancedFlashUploader"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementFileAdvancedFlashUploaderMulti"><?php echo BFText::_('FLASH_UPLOADER_MULTI'); ?></label>
			            			<input type="checkbox" value="" id="bfElementFileAdvancedFlashUploaderMulti"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementFileAdvancedFlashUploaderBytes"><?php echo BFText::_('FLASH_UPLOADER_BYTES'); ?></label>
			            			<input type="text" value="" id="bfElementFileAdvancedFlashUploaderBytes"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementFileAdvancedFlashUploaderWidth"><?php echo BFText::_('FLASH_UPLOADER_WIDTH'); ?></label>
			            			<input type="text" value="" id="bfElementFileAdvancedFlashUploaderWidth"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementFileAdvancedFlashUploaderHeight"><?php echo BFText::_('FLASH_UPLOADER_HEIGHT'); ?></label>
			            			<input type="text" value="" id="bfElementFileAdvancedFlashUploaderHeight"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementFileAdvancedFlashUploaderTransparent"><?php echo BFText::_('FLASH_UPLOADER_TRANSPARENT'); ?></label>
			            			<input type="checkbox" value="" id="bfElementFileAdvancedFlashUploaderTransparent"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementFileAdvancedUploadDirectory"><?php echo BFText::_('UPLOAD_DIRECTORY'); ?></label>
			            			<input type="text" value="" id="bfElementFileAdvancedUploadDirectory"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementFileAdvancedTimestamp"><?php echo BFText::_('TIMESTAMP'); ?></label>
			            			<input checked="checked" type="checkbox" value="" id="bfElementFileAdvancedTimestamp"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementFileAdvancedAllowedFileExtensions"><?php echo BFText::_('ALLOWED_FILE_EXTENSIONS'); ?></label>
			            			<input type="text" value="" id="bfElementFileAdvancedAllowedFileExtensions"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementFileAdvancedAttachToUserMail"><?php echo BFText::_('ATTACH_TO_USERMAIL'); ?></label>
			            			<input checked="checked" type="checkbox" value="" id="bfElementFileAdvancedAttachToUserMail"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementFileAdvancedAttachToAdminMail"><?php echo BFText::_('ATTACH_TO_ADMINMAIL'); ?></label>
			            			<input checked="checked" type="checkbox" value="" id="bfElementFileAdvancedAttachToAdminMail"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementFileAdvancedHideLabel"><?php echo BFText::_('HIDE_LABEL'); ?></label>
			            			<input checked="checked" type="checkbox" value="" id="bfElementFileAdvancedHideLabel"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementFileAdvancedLogging"><?php echo BFText::_('LOGGING'); ?></label>
			            			<input checked="checked" type="checkbox" value="" id="bfElementFileAdvancedLogging"/>
			            			<br/>
			            			<br/>
			            			<label class="bfPropertyLabel" for="bfElementFileAdvancedOrderNumber"><?php echo BFText::_('ORDER_NUMBER'); ?></label>
			            			<input type="text" value="" id="bfElementFileAdvancedOrderNumber"/>
			            		</div>
			            		<!-- FILE END -->
			            		<div id="bfAdvancedLeaf">
				            		<br/>
				            		<label class="bfPropertyLabel" id="bfElementAdvancedTabIndexLabel" for="bfElementAdvancedTabIndex"><?php echo BFText::_('TAB_INDEX'); ?></label>
				            		<input type="text" value="" id="bfElementAdvancedTabIndex"/>
				            		<br/>
				            		<br/>
				            		<label class="bfPropertyLabel" for="bfElementAdvancedTurnOff"><?php echo BFText::_('TURN_OFF_INITIALLY'); ?></label>
			            			<input type="checkbox" value="" id="bfElementAdvancedTurnOff"/>
				            		<br/>
				            		<br/>
				            		<label class="bfPropertyLabel" id="bfElementAdvancedLabelPositionLabel" for="bfElementAdvancedLabelPosition"><?php echo BFText::_('LABEL_POSITION'); ?></label>
				            		<select id="bfElementAdvancedLabelPosition">
				            			<option value="left"><?php echo BFText::_('LEFT'); ?></option>
				            			<option value="top"><?php echo BFText::_('TOP'); ?></option>
				            			<option value="right"><?php echo BFText::_('RIGHT'); ?></option>
				            			<option value="bottom"><?php echo BFText::_('BOTTOM'); ?></option>
				            		</select>
			            		</div>
			            	</fieldset>
			            	...
			            	<fieldset id="bfInitScript" style="display:none">
			            		<br/>
			            		<legend><?php echo BFText::_('ADVANCED_ELEMENT_INITSCRIPT'); ?></legend>
			            		<?php echo BFText::_('TYPE') ?>:
								...
								<div id="bfInitScriptFlags" style="display:none">
									<hr/>
									<input type="checkbox" id="bfInitFormEntry" class="bfInitFormEntry" name="bfInitFormEntry" value="1"/><label for="bfInitFormEntry"> <?php echo BFText::_('ELEMENTS_FORMENTRY'); ?></label>
									<input type="checkbox" id="bfInitPageEntry" class="bfInitPageEntry" name="bfInitPageEntry" value="1"/><label for="bfInitPageEntry"> <?php echo BFText::_('ELEMENTS_PAGEENTRY'); ?></label>
								</div>
								...
								<div id="bfInitScriptCustom" style="display:none">
									<hr/>
									<div style="cursor: pointer;" onclick="createInitCode()"><?php echo BFText::_('CREATE_CODE_FRAMEWORK') ?></div>
			            	...
			            	<fieldset id="bfActionScript" style="display:none">
			            		<br/>
			            		<legend><?php echo BFText::_('ADVANCED_ELEMENT_ACTIONSCRIPT'); ?></legend>
								...
								<div id="bfActionScriptFlags" style="display:none">
									...
									<input style="display:none" type="checkbox" class="bfAction" id="bfActionClick" name="bfActionClick" value="1"/><label style="display:none" class="bfActionLabel" id="bfActionClickLabel"> <?php echo BFText::_('ELEMENTS_CLICK'); ?></label>
									<input style="display:none" type="checkbox" class="bfAction" id="bfActionBlur" name="bfActionBlur" value="1"/><label style="display:none" class="bfActionLabel" id="bfActionBlurLabel"> <?php echo BFText::_('ELEMENTS_BLUR'); ?></label>
									<input style="display:none" type="checkbox" class="bfAction" id="bfActionChange" name="bfActionChange" value="1"/><label style="display:none" class="bfActionLabel" id="bfActionChangeLabel"> <?php echo BFText::_('ELEMENTS_CHANGE'); ?></label>
									<input style="display:none" type="checkbox" class="bfAction" id="bfActionFocus" name="bfActionFocus" value="1"/><label style="display:none" class="bfActionLabel" id="bfActionFocusLabel"> <?php echo BFText::_('ELEMENTS_FOCUS'); ?></label>
									<input style="display:none" type="checkbox" class="bfAction" id="bfActionSelect" name="bfActionSelect" value="1"/><label style="display:none" class="bfActionLabel" id="bfActionSelectLabel"> <?php echo BFText::_('ELEMENTS_SELECTION'); ?></label>
								</div>
								...
								<div id="bfActionScriptCustom" style="display:none">
									<hr/>
									<div style="cursor: pointer;" onclick="createActionCode()"><?php echo BFText::_('CREATE_CODE_FRAMEWORK') ?></div>
			            ...
			            <div class="bfFadingMessage" style="display:none"></div>
			            <input type="submit" value="<?php echo BFText::_('PROPERTIES_SAVE'); ?>" id="bfAdvancedSaveButton"/>
	            	</div>
	            </div>
            </div>
