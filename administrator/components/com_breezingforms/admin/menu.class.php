...
class facileFormsMenu
{
	function create($option, $pkg)
	{
		...
		$parents = array();
		$parents[] = mosHTML::makeOption(0, BFText::_('MENUS_TOP'));
		$database->setQuery(
	...
	function save($option, $pkg)
	{
		...
		mosRedirect("index2.php?option=$option&act=managemenus&pkg=$pkg", BFText::_('MENUS_SAVED'));
	} // save
	...
	function copy($option, $pkg, $ids)
	{
		...
		$msg = $total.' '.BFText::_('MENUS_SUCOPIED');
		updateComponentMenus();
	...
	function listitems($option, $pkg)
	{
		...
		$database->setQuery(
			...
			"where m.package =  '".mysql_escape_string($pkg)."' ".
			"order by ".
