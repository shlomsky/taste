...
class facileFormsForm
{
	...
	function save($option, $pkg, $caller)
	{
		...
		if($pkg != 'EasyModeForms' && $pkg != 'QuickModeForms'){
			mosRedirect($caller, BFText::_('FORMS_SAVED'));
		} else {
			
			ob_start();
			ob_end_clean();
			if($pkg == 'EasyModeForms'){
				echo '<html><head><script>parent.SqueezeBox.close();</script></head><body></body></html>';
			} else if($pkg == 'QuickModeForms'){
				echo '<html><head><script>parent.SqueezeBox.close();parent.location.href="index2.php?option=com_breezingforms&act=quickmode&formName='.$row->name.'&form='.$row->id.'"</script></head><body></body></html>';
			}
	...
	function copy($option, $pkg, $ids)
	{
		...
		$msg = $total.' '.BFText::_('FORMS_SUCOPIED');
