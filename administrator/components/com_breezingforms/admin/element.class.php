...
class facileFormsElement
{
	...
	function getDestination($option, $pkg, $form, $page, $ids, $action)
	{
		...
		if (count($rows)) foreach ($rows as $row) {
			...
			for ($p = 1; $p <= $row->pages; $p++)
				$fff[] = mosHTML::makeOption($row->id.','.$p, $row->package.'::'.$row->name.' / '.BFText::_('ELEMENTS_PAGE').' '.$p);
		} // for
	...
	function copy($option, $pkg, $form, $page, $ids)
	{
		...
		if (!$newform && !$newpage)
			mosRedirect("index2.php?option=$option&act=editpage&form=$form&page=$page&pkg=$pkg&mosmsg=".BFText::_('ELEMENTS_ANERROR'));
		$total = count($ids);
		$row = new facileFormsElements($database);
		...
		$msg = $total. ' '.BFText::_('ELEMENTS_COPIED').$newform.', '.BFText::_('ELEMENTS_PAGE2').$newpage;
	...
	function move($option, $pkg, $form, $page, $ids)
	{
		...
		if ($newform != $form || $newpage != $page) {
			...
			$msg = $total. ' '.BFText::_('ELEMENTS_MOVED').$newform.', '.BFText::_('ELEMENTS_PAGE2').$newpage;
		} else
			$msg = BFText::_('ELEMENTS_NOTMOVED');
		mosRedirect("index2.php?option=$option&act=editpage&form=$form&page=$page&pkg=$pkg&mosmsg=$msg");
	} // move
	...
	function getPageDestination($option, $pkg, $form, $page)
	{
		...
		$lst = array();
		for($p = 1; $p <= $row->pages; $p++)
			$lst[] = mosHTML::makeOption($p, BFText::_('ELEMENTS_PAGE').' '.$p);
		$sellist =
			mosHTML::selectList(
