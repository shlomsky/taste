...
function BF_EasyModeApp()
{
		...
		this.populateHiddenFieldsOptions = function(){
			...
			for(var i=0;i < hiddenFields.length;i++){
				...
				element.innerHTML = '<?php echo BFText::_('Name') ?>:';
			...
			if(hiddenFields.length != 0){
				...
				JQuery(element).click(
					function(){
						...
						JQuery('#bfHiddenFieldsOptions').get(0).innerHTML = '<?php echo BFText::_('Options saved! To finally save your form, click "save" on the top right button!') ?><br/><br/>' + JQuery('#bfHiddenFieldsOptions').get(0).innerHTML;
					}
				);
		...
		*/
		this.prepareForSave = function(){
			...
			if(appScope.captchaAdded > 1){
				alert('<?php echo BFText::_('You added more than one captcha. Please remove the element in order to be able to save!') ?>');
				return;
			}
		...
		this.createElementByType = function(typeName, rndId){
			...
			switch(typeName){
				...
				case 'bfCaptcha':
					...
					imageHolder.setAttribute('src', '<?php echo JURI::root() ?>index.php?raw=true&option=com_breezingforms&bfCaptcha=true');
					...
					reload.innerHTML = '<a href="#" onclick="document.getElementById(\'bfCaptchaEntry\').value=\'\';document.getElementById(\'bfCaptchaEntry\').focus();document.getElementById(\'ff_capimgValue\').src = \'<?php echo JURI::root() ?>index.php?raw=true&option=com_breezingforms&bfCaptcha=true?\' + Math.random(); return false"><img src="<?php echo JURI::root() ?>components/com_breezingforms/images/captcha/refresh-captcha.png" border="0" /></a>';
				...
				case 'bfIcon':
					...
					var iconImage = '<?php echo JURI::root() ?>administrator/images/next.png';
					...
					data3 = '<?php echo JURI::root() ?>administrator/images/next_f2.png',
					data1 = iconImage;
				...
				case 'bfTooltip':
					...
					element.setAttribute('name', 'ff_nm_' + rndId + '[]');
					element.setAttribute('class', 'ff_elem');
					element.setAttribute('src', '<?php echo JURI::root() ?>includes/js/ThemeOffice/tooltip.png');
					JQuery(element).click(appScope.resizableElement);
		...
		*/  
		...
		JQuery('#bfDeleteThisPage').click(
			function(){
				...
				if(!confirm('<?php echo BFText::_('Are you sure you want to delete this page?') ?>')){
					return;
				}
		...
		JQuery('#bfInitButton').click(
			function(){
				...
				JQuery('.bfScriptsSaveMessage').empty().append('<?php echo BFText::_('Script saved! To finally save your form, click "save" on the top right button!') ?><br/><br/>');
			}
		);
		...
		JQuery('#bfSaveOptionsButton').click(
			function(){
					...
					switch(appScope.optionElement.internalType){
						...
						case 'bfStaticText':
							if(JQuery.trim(JQuery('#bfStaticTextTitle').get(0).value) == ''){
								error = '<?php echo BFText::_('Title must not be empty!') ?>';
							}
						...
						case 'bfIcon':
							if(JQuery.trim(JQuery('#bfIconCaption').get(0).value) == ''){
								error = '<?php echo BFText::_('Caption must not be empty!') ?>';
							}
							if(JQuery.trim(JQuery('#bfIconImage').get(0).value) == ''){
								error = '<?php echo BFText::_('Icon Image must not be empty!') ?>';
							}
							if(JQuery.trim(JQuery('#bfIconImageOver').get(0).value) == ''){
								error = '<?php echo BFText::_('Icon Image Over must not be empty!') ?>';
							}
							if(error == ''){
						...
						case 'bfTextfield':
							...
							{
								error = '<?php echo BFText::_('Name must not be empty!') ?>';
							}
							if(error == ''){
				...
				if(error == ''){
					JQuery('#bfOptionsSaveMessage').get(0).innerHTML = '<?php echo BFText::_('Options saved! To finally save your form, click "save" on the top right button!') ?><br/><br/>';
				} else {
...
function submitbutton(pressbutton)
{
		...
		switch (pressbutton) {
			...
			case 'editform':
			    ...
			    SqueezeBox.loadModal("index3.php?option=com_breezingforms&task=editform&act=editpage&form=<?php echo $formId ?>&pkg=EasyModeForms","iframe",820,400);
			case 'preview':
				SqueezeBox.initialize({});               
			    ...
			    SqueezeBox.loadModal("<?php echo JURI::root()?>index2.php?format=html&option=com_breezingforms&ff_form=<?php echo $formId ?>&ff_page=<?php echo $page ?>","iframe",820,400);
				break; 
			case 'preview_site':
				SqueezeBox.initialize({});               
