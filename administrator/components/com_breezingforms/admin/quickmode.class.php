...
class QuickMode{
	...
	function __construct(){
		...
		$this->db->setQuery("Select id From #__facileforms_packages Where name = 'QuickModeForms' And id = 'QuickModeForms'");
		if(count($this->db->loadObjectList()) == 0){
			$this->db->setQuery(
				...
				(
					...
					'1.0',
					'".date('Y-m-d H:i:s')."',
					'Forms for the quick mode',
					'Markus Bopp',
					'markus.bopp@crosstec.de',
					'http://www.crosstec.de',
					'All easy forms go here',
					'Copyright depending on the author of the forms included in this package.'
				)"
			);
			$this->db->query();
		}
	...
	public function save2($form, $formName, $formTitle, $formDescription, $templateCode, array $areas, $pages = 1){
		...
		$this->db->setQuery(
						"Update 
							#__facileforms_forms
						 Set
						 	template_code = ".$this->db->Quote( base64_encode( Zend_Json::encode( $dataObject ) ) ).",
	...
	public function getFormOptions($form){
		$this->db->setQuery("Select name, title, description, emailntf, emailadr From #__facileforms_forms Where id = ".$this->db->Quote($form)."");
		$objList = $this->db->loadObjectList();
		$objListCount = count($objList);
		if($objListCount == 1){
			return $objList[0];
