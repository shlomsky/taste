...
class ff_importPackage extends ff_xmlPackage
{
	...
	function import($filename)
	{
		...
		if (!$ok) {
			...
			if ($errmode=='log') {
				...
				$errors[] = BFText::_('INSTALLER').': '.$this->error;
			} // if
		} else {
			// success case
	...
	function emitScript()
	{
		...
		$row->name        = $this->getText(1, 'name', BFText::_('INSTALLER_UNKNOWN'));
		$row->title       = $this->getText(1, 'title', BFText::_('INSTALLER_UNKNOWN'));
				...
				if ($id == NULL) {
					...
					if ($id == NULL) {
						...
						if ($id == NULL) {
							...
							$cond = 2;
							$code = '// '.$tag.' '.$idx.'/'.$name.' '.BFText::_('INSTALLER_TAGNOTFOUND');
							$nm   = $this->getText($pid, 'name');
							$this->warnings[] = $nm.': '.$tag.' '.$idx.'/'.$name.' '.BFText::_('INSTALLER_TAGNOTFOUND');
						} // if
					} // if
				} // if
				break;
	...
	function emitElement()
	{
		...
		$row->title     = $this->getText(2, 'title', BFText::_('INSTALLER_UNKNOWN'));
		$row->name      = $this->getText(2, 'name', BFText::_('INSTALLER_UNKNOWN'));
