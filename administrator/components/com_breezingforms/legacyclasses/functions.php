...
function mosNotAuth()
{
	...
	echo BFText::_('ALERTNOTAUTH');
	if ($user->get('id') < 1) {
		echo "<br />" . BFText::_( 'You need to login.' );
	}
}
}
/**
...
function mosShowSource( $filename, $withLineNums=false ) {
	...
	if (!($source = @highlight_file( $filename, true ))) {
		return BFText::_( 'Operation Failed' );
	}
...
function mosFormatDate( $date = 'now', $format = null, $offset = null )  {
	...
	if ( ! $format )
	{
		$format = BFText::_('DATE_FORMAT_LC1');
	}
...
function josSpoofCheck( $header=false, $alternate=false )
{
	...
	if (!$check)
	{
		...
		jexit( BFText::_('E_SESSION_TIMEOUT') );
	}
}
}
/**
